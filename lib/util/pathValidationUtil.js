...
export function validateLink(link, filename, fullPath, directory, lineNumber) {
	...
	if (link.startsWith('/')) {
		...
		const isFileSystemAbsolute = path.isAbsolute(link) && !link.startsWith('/');
		const isOutsideRepo = relativePath === '..' || relativePath.startsWith('..' + path.sep);

		if (isFileSystemAbsolute) {
			printHeader(' Absolute File System Path Used ');
		...
		} else if (isOutsideRepo) {
			printHeader(' Path Outside Repository ');
