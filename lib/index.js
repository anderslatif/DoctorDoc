...
// Allows writing `doctordoc browse [path]` instead of `doctordoc --browse [path]`
program
.command('browse [path]')
.description('Open repository URL for file/directory. Defaults to the current directory if no path provided.')
.action((targetPath) => {
  program.setOptionValue('browse', targetPath || true);
});
